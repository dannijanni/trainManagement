...
const AnalyticsDashboard: React.FC = () => {
  ...
  useEffect(() => {
    calculateAnalytics();
  }, [bookings, trains, users, drivers, routes, dateRange]);
  ...
  const calculateAnalytics = () => {
    ...
    const totalSeats = trains.reduce((sum, train) => 
      sum + Object.values(train.classes).reduce((classSum, cls) => classSum + cls.totalSeats, 0), 0
    );
    const bookedSeats = filteredBookings.reduce((sum, booking) => sum + booking.seats.length, 0);
    ...
    const routeBookings = filteredBookings.reduce((acc, booking) => {
      ...
      if (train) {
        ...
        acc[routeKey].revenue += booking.totalAmount;
      }
      return acc;
    ...
    const routePerformance = routes
      ...
      .map(route => {
        ...
        const revenue = routeBookings.reduce((sum, b) => sum + b.totalAmount, 0);
        const totalCapacity = trains
          .filter(t => t.route.from === route.from && t.route.to === route.to)
          .reduce((sum, train) => 
            sum + Object.values(train.classes).reduce((classSum, cls) => classSum + cls.totalSeats, 0), 0
          );
        const bookedSeats = routeBookings.reduce((sum, booking) => sum + booking.seats.length, 0);
                  ...
                  <div 
                    ...
                    style={{ 
                      width: `${(analytics.customerDemographics.returningCustomers / 
                        (analytics.customerDemographics.newCustomers + analytics.customerDemographics.returningCustomers)) * 100}%` 
                    }}
                  ></div>
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  {((analytics.customerDemographics.returningCustomers / 
                    (analytics.customerDemographics.newCustomers + analytics.customerDemographics.returningCustomers)) * 100).toFixed(1)}% 
                  returning customers
                </p>
              </div>
            </div>
